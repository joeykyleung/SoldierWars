...
public class Soldier_wars extends Applet implements Runnable, KeyListener, MouseMotionListener, MouseListener {
   ...
   public void init() {
      ...
      tankhitPic = getImage(getDocumentBase(), "tankhit.png");
      ...
      cannonSprite = getImage(getDocumentBase(), "cannonSprite.png");
      explosionSprite = getImage(getDocumentBase(), "explosionSprite.png");
      nukeSprite = getImage(getDocumentBase(), "nuke.png");
      bossboomSprite = getImage(getDocumentBase(), "bossboom.png");
      ...
      if (gameStart == true) {
         ...
         if (levelCount == 5 || levelCount == 4 || levelCount == 6 || tutorial == true) {
            ...
            for (int x = 0; x < airstrike.length; x++) {
               ...
               if (airstrike[x].boomStatus == true) {
                  ...
                  sy1 = airstrike[x].firstFrameYPos + 60;
                  sy2 = sy1 + airstrike[x].frameHeight;
                  sx1 = airstrike[x].firstFrameXPos + airstrike[x].frameCounter * airstrike[x].frameWidth;
                  sx2 = sx1 + airstrike[x].frameWidth;
               
                  dx1 = airstrike[x].xpos;
                  dy1 = airstrike[x].ypos - 105;
                  dx2 = dx1 + airstrike[x].frameWidth + 20;
                  dy2 = dy1 + airstrike[x].frameHeight + 20;
               
                  airstrike[x].threadCounter++;
               
                  if (airstrike[x].threadCounter > 2) {
                     airstrike[x].threadCounter = 0;
                     airstrike[x].frameCounter++;
                  }
               
                  bufferGraphics.drawImage(nukeSprite, dx1, dy1, dx2, dy2, sx1, sy1, sx2, sy2, this);
                  airstrike[x].boomTime--;
               
                  if (airstrike[x].frameCounter > airstrike[x].frames - 1) {
            ...
            for (int e = 0; e < jet.length; e++) {
               ...
               if (jet[e].explodeStatus == true) {
                  ...
                  sy1 = jet[e].firstFrameYPos;
                  sy2 = sy1 + jet[e].frameHeight;
                  sx1 = jet[e].firstFrameXPos + jet[e].frameCounter * jet[e].frameWidth - 20;
                  sx2 = sx1 + jet[e].frameWidth;
               
                  dx1 = jet[e].xpos;
                  dy1 = jet[e].ypos;
                  dx2 = dx1 + jet[e].frameWidth + 40;
                  dy2 = dy1 + jet[e].frameHeight + 40;
               
                  jet[e].threadCounter++;
               
                  if (jet[e].threadCounter > 1) {
                     jet[e].threadCounter = 0;
                     jet[e].frameCounter++;
                  }
               
                  if (jet[e].frameCounter > jet[e].frames - 1) {
                     jet[e].explodeStatus = false;
                  }
               
                  bufferGraphics.drawImage(explosionSprite, dx1, dy1, dx2, dy2, sx1, sy1, sx2, sy2, this);
               }
            }
         }
         ...
         if (levelCount == 6) {
            ...
            if (boss.explodeStatus == true) {
               ...
               sy1 = boss.firstFrameYPos;
               sy2 = sy1 + boss.frameHeight;
               sx1 = boss.firstFrameXPos + boss.frameCounter * boss.frameWidth;
               sx2 = sx1 + boss.frameWidth;
            
               dx1 = boss.xpos;
               dy1 = boss.ypos;
               dx2 = dx1 + boss.frameWidth + 40;
               dy2 = dy1 + boss.frameHeight + 40;
            
               boss.threadCounter++;
            
               if (boss.threadCounter > 1) {
                  boss.threadCounter = 0;
                  boss.frameCounter++;
               }
            
               if (boss.frameCounter > boss.frames - 1) {
                  boss.explodeStatus = false;
               }
            
               bufferGraphics.drawImage(bossboomSprite, dx1, dy1, dx2, dy2, sx1, sy1, sx2, sy2, this);
               if (boss.explodeTime == 39) {
         ...
         for (int x = 0; x < cannon.length; x++) {
            if (cannon[x].isAlive == true) {
               ...
               sy1 = cannon[x].firstFrameYPos;
               sy2 = sy1 + cannon[x].frameHeight;
               sx1 = cannon[x].firstFrameXPos + cannon[x].frameCounter * cannon[x].frameWidth + 1;
               sx2 = sx1 + cannon[x].frameWidth;
            
               dx1 = cannon[x].xpos - 10;
               dy1 = cannon[x].ypos + 20;
               dx2 = dx1 + cannon[x].frameWidth;
               dy2 = dy1 + cannon[x].frameHeight;
            
               cannon[x].threadCounter++;
            
               if (cannon[x].threadCounter > 2) {
                  cannon[x].threadCounter = 0;
                  cannon[x].frameCounter++;
               }
            
               if (cannon[x].frameCounter > cannon[x].frames - 1) {
                  cannon[x].frameCounter = 0;
               }
            
               bufferGraphics.drawImage(cannonSprite, dx1, dy1, dx2, dy2, sx1, sy1, sx2, sy2, this);
            }
         }
         ...
         if (tank.isAlive == true) {
            ...
            if (tankhit == true) {
               tanktimer--;
               bufferGraphics.drawImage(tankhitPic, tank.xpos, tank.ypos, tank.width, tank.height, this);
               if (tanktimer == 0) {
                  tankhit = false;
                  tanktimer = 10;
               }
